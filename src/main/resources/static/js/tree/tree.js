...
$(function() {
  let compareInt = function(a, b) {
    return a - b;
  };

  let scatteringMap = Array.from(
    new Set(features.map(feature => feature.featureScattering))
  ).sort(compareInt);
  $("#slider-scattering").slider({
    orientation: "vertical",
    range: true,
    values: [0, scatteringMap[scatteringMap.length - 1]],
    min: 0,
    max: scatteringMap.length - 1,
    slide: function(event, ui) {
      $("#amountScattering").val(
        scatteringMap[ui.values[0]] + " - " + scatteringMap[ui.values[1]]
      );
    },
    stop: function(event, ui) {
      let featuresFiltered = features.filter(
        feature =>
          feature.featureScattering >= scatteringMap[ui.values[0]] &&
          feature.featureScattering <= scatteringMap[ui.values[1]]
      );
      featuresFiltered = featuresFiltered.filter(
        feature =>
          feature.tangling >=
            tanglingMap[$("#slider-tangling").slider("values")[0]] &&
          feature.tangling <=
      ...
      $.ajax({
        type: "POST",
        url: path + "/newick/filtered",
        data: JSON.stringify(featureIds),
        success: function(data) {
          var labelscales = "";
          featuresFiltered.forEach(function(element, index, array) {
            labelscales +=
              element.name +
              ":" +
              Math.round(xScaleScattering(element.featureScattering) * 100) /
                100;
  ...
  $("#amountScattering").val(
    1 + " - " + scatteringMap[(0, scatteringMap.length - 1)]
  );

  let tanglingMap = Array.from(
    new Set(features.map(feature => feature.tangling))
  ).sort(compareInt);
  $("#slider-tangling").slider({
    orientation: "vertical",
    range: true,
    values: [0, tanglingMap[tanglingMap.length - 1]],
    min: 0,
    max: tanglingMap.length - 1,
    slide: function(event, ui) {
      $("#amountTangling").val(
        tanglingMap[ui.values[0]] + " - " + tanglingMap[ui.values[1]]
      );
    },
    stop: function(event, ui) {
      let featuresFiltered = features.filter(
        feature =>
          feature.tangling >= tanglingMap[ui.values[0]] &&
          feature.tangling <= tanglingMap[ui.values[1]]
      );
      featuresFiltered = featuresFiltered.filter(
        feature =>
          feature.featureScattering >=
            scatteringMap[$("#slider-scattering").slider("values")[0]] &&
          feature.featureScattering <=
            scatteringMap[$("#slider-scattering").slider("values")[1]]
      );

      const featureIds = featuresFiltered.map(feature => feature.id);
      $.ajax({
        type: "POST",
        url: path + "/newick/filtered",
        data: JSON.stringify(featureIds),
  ...
  $("#amountTangling").val(
    1 + " - " + tanglingMap[(0, tanglingMap.length - 1)]
  );
});

(function() {
  let Node, Tree, equal_angle, root;
  ...
  Tree = function(s) {
    let flag, item, j, len, node, ref, ref1, ref2, token, tree;
    ...
    for (j = 0, len = token.length; j < len; j++) {
      ...
      switch (item) {
        ...
        case ",":
          ...
          (ref2 = [node.father.addChild(), false]),
            (node = ref2[0]),
            (flag = ref2[1]);
          break;
        case ":":
