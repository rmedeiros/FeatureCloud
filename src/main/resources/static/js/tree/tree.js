...
$(function() {
  ...
  $("#slider-scattering").slider({
    ...
    slide: function(event, ui) {
      $("#amountScattering").val(scatteringMap[ui.values[0]] + " - " + scatteringMap[ui.values[1]]);
    },
    stop: function(event, ui) {
      var featuresFiltered = features.filter(
        feature =>
      ...
      featuresFiltered = featuresFiltered.filter(
        feature =>
          feature.tangling >= tanglingMap[$("#slider-tangling").slider("value")]
      );
      var featureIds = featuresFiltered.map(feature => feature.id);
      $.ajax({
        type: "POST",
        url: "/newick/filtered",
  ...
  $("#slider-tangling").slider({
    ...
    value: 0,
    slide: function(event, ui) {
      $("#amountTangling").val(tanglingMap[ui.value]);
    },
    stop: function(event, ui) {
      var featuresFiltered = features.filter(
        feature => feature.tangling >= tanglingMap[ui.value]
      );
      featuresFiltered = featuresFiltered.filter(
        feature =>
          feature.featureScattering >=
          scatteringMap[$("#slider-scattering").slider("value")]
      );
