...
$(function () {
    ...
    var scatteringMap = Array.from(new Set(features.map(feature = > feature.featureScattering))
).
    sort(compareInt);
    $("#slider-scattering").slider({
        ...
        slide: function (event, ui) {
            $("#amountScattering").val(scatteringMap[ui.value]);
        },
        stop: function (event, ui) {
            var featuresFiltered = features.filter(feature = > feature.featureScattering >= scatteringMap[ui.value]
        )
            ;
            featuresFiltered = featuresFiltered.filter(feature = > feature.tangling >= tanglingMap[$("#slider-tangling").slider("value")]
        )
            ;
    ...
    $("#amountScattering").val(1);

    var tanglingMap = Array.from(new Set(features.map(feature = > feature.tangling))
).
    sort(compareInt);
    $("#slider-tangling").slider({
        ...
        stop: function (event, ui) {
            var featuresFiltered = features.filter(feature = > feature.tangling >= tanglingMap[ui.value]
        )
            ;
            featuresFiltered = featuresFiltered.filter(feature = > feature.featureScattering >= scatteringMap[$("#slider-scattering").slider("value")]
        )
            ;
...
(function () {
    ...
    equal_angle = function (tree_string, length_scale, equal_length) {
        var angle, c, i, j, k, len, len1, len2, length, m, move_along, n, o, postlist, present_langle, ref, ref1, ref2,
            total_angle, tree;
